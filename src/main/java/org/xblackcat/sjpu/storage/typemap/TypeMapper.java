...
public class TypeMapper {
    static final Map<Class<?>, String> READ_DECLARATIONS = Map.ofEntries(
            ...
            // Float types
            Map.entry(double.class, "double value%1$d = $1.getDouble(%1$d);\n"),
            Map.entry(
                    Double.class,
                    "double tmp%1$d = $1.getDouble(%1$d);\njava.lang.Double value%1$d = $1.wasNull() ? null : java.lang.Double.valueOf(tmp%1$d);\n"
            ),
            Map.entry(float.class, "float value%1$d = $1.getFloat(%1$d);\n"),
            Map.entry(
                    Float.class,
            ...
            // Boolean type
            Map.entry(boolean.class, "boolean value%1$d = $1.getBoolean(%1$d);\n"),
            Map.entry(
                    Boolean.class,
            ...
            // Other types
            Map.entry(byte[].class, "byte[] value%1$d = $1.getBytes(%1$d);\n"),
            Map.entry(String.class, String.class.getName() + " value%1$d = $1.getString(%1$d);\n"),
            Map.entry(
                    BigDecimal.class,
                    BigDecimal.class.getName() + " value%1$d = $1.getBigDecimal(%1$d);\n"
            ),
            Map.entry(Object.class, Object.class.getName() + " value%1$d = $1.getObject(%1$d);\n"),
    ...
    public synchronized ITypeMap<?, ?> hasTypeMap(Class<?> objClass) {
        ...
        for (IMapFactory<?, ?> typeMapper : mappers) {
            if (typeMapper.isAccepted(objClass)) {
                @SuppressWarnings("unchecked") final ITypeMap<?, ?> typeMap = typeMapper.mapper((Class) objClass);
                ...
                try {
                    ...
                    @SuppressWarnings("rawtypes") final Class aClass = instanceClass.toClass(TypeMapper.class);
    ...
    public synchronized Class<IToObjectConverter<?>> getOrInitTypeMap(
        ...
        try {
            ...
            if (IToObjectConverter.class.isAssignableFrom(aClass)) {
                ...
                @SuppressWarnings("unchecked") final Class<IToObjectConverter<?>> aClazz = (Class<IToObjectConverter<?>>) aClass;
                return aClazz;
            } else {
        ...
        @SuppressWarnings("unchecked") final Class<IToObjectConverter<?>> converterClass = (Class<IToObjectConverter<?>>) toObjectConverter.toClass(
                targetClass);
